...
const { getLastResult, getQualifyingLap, getLastQualifyingResult, searchForDriver } = require('./util.js');
...
const LastPoleIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        try {
            ...
            speakOutput = `${data.results[0].driver_nationality} driver ${data.results[0].driver_forename} ${data.results[0].driver_surname} started on pole position at the ${data.last_race_year}
            ${data.last_race_name} grand prix after a lap time of ${lapTime}.`
        } catch(e) {
            console.error(`Error fetching result for LastPoleIntentHandler: ${e}`);
            speakOutput = 'I was unable to retrieve race result information at this time. Please check back later'
        }
        return handlerInput.responseBuilder
            .speak(speakOutput)
    ...
    async handle(handlerInput) {
        ...
        try {
            ...
            if (!searchedResult) {
                ...
                speakOutput = `${searchedResult.driver_nationality} driver ${searchedResult.driver_forename} ${searchedResult.driver_surname} qualified
                in <say-as interpret-as="ordinal">${searchedResult.qualifying_position}</say-as> at the ${result.data.last_race_year} ${result.data.last_race_name} grand prix,
                with a lap time of ${lapTime}`;
            }
        } catch(e) {
            console.error(`Error fetching result for LastRacePositionIntentHandler: ${e}`);
            speakOutput = 'I was unable to retrieve race result information at this time. Please check back later'
...
const GetFullRaceResultIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        try {
            ...
            console.error(`Error fetching result for GetFullRaceResultIntentHandlerHandler: ${e}`);
            speakOutput = 'I was unable to retrieve race result information at this time. Please check back later'
ˇ
        }
    }
...
const GetFullQualifyingResultIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        try {
            const result = await getLastQualifyingResult();
            const data = result.data;
            speakOutput = `Here are the full results of the ${data.last_race_year} ${data.last_race_name} grand prix qualifying session. `
            for (let i = 0; i < config.listMax; i++) {
                if (i < data.results.length) {
                    speakOutput += `<amazon:emotion name="excited" intensity="medium"><break time="1s"/><say-as interpret-as="ordinal">${i + 1}</say-as> ${data.results[i].driver_forename} ${data.results[i].driver_surname}</amazon:emotion>`;
                }
            ...
            console.error(`Error fetching result for GetFullQualifyingResultIntentHandler: ${e}`);
            speakOutput = 'I was unable to retrieve race result information at this time. Please check back later'
ˇ
        }
    }
