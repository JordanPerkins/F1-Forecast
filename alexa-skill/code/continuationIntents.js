                ...
                for (let i = attributes.lastPosition; i < attributes.lastPosition+config.listMax; i++) {
                    if (i < attributes.lastResult.length) {
                        if (attributes.lastIntent === 'ReadRaceForecastIntent' || attributes.lastIntent === 'ReadQualifyingForecastIntent' || attributes.lastIntent === 'GetFullRaceResultIntent' || attributes.lastIntent === 'GetFullQualifyingResultIntent') {
                            ...
                            speakOutput += `<amazon:emotion name="excited" intensity="medium"><break time="1s"/><say-as interpret-as="ordinal">${i + 1}</say-as> ${attributes.lastResult[i].driver_forename} ${attributes.lastResult[i].driver_surname} with ${attributes.lastResult[i].driver_points} point${attributes.lastResult[i].driver_points === 1 ? '' : 's'}.</amazon:emotion>`;  
                        } else if (attributes.lastIntent === 'FullChampionshipConstructorsIntent') {
                            speakOutput += `<amazon:emotion name="excited" intensity="medium"><break time="1s"/><say-as interpret-as="ordinal">${i + 1}</say-as> ${attributes.lastResult[i].constructor_name} with ${attributes.lastResult[i].constructor_points} point${attributes.lastResult[i].constructor_points === 1 ? '' : 's'}.</amazon:emotion>`;  
                        }
                    }
                }
