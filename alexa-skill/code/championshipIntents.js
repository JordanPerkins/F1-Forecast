...
const ChampionshipLeaderIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        try {
            ...
            speakOutput = `The driver's championship is currently led by ${driversLeader.driver_nationality} driver ${driversLeader.driver_forename} ${driversLeader.driver_surname} with ${driversLeader.driver_points} point${driversLeader.driver_points === 1 ? '' : 's'} and ${driversLeader.driver_wins} win${driversLeader.driver_wins === 1 ? '' : 's'}. `;
            speakOutput += `The constructor's championship is currently led by ${constructorsLeader.constructor_nationality} constructor ${constructorsLeader.constructor_name} with ${constructorsLeader.constructor_points} point${constructorsLeader.constructor_points === 1 ? '' : 's'} and ${constructorsLeader.constructor_wins} win${constructorsLeader.constructor_wins === 1 ? '' : 's'}.`
        } catch(e) {
    ...
    async handle(handlerInput) {
        ...
        try {
            ...
            if (!searchedResult) {
                ...
                <say-as interpret-as="ordinal">${searchedResult.driver_position}</say-as> in the championship, with ${searchedResult.driver_points} point${searchedResult.driver_points === 1 ? '' : 's'} and ${searchedResult.driver_wins} win${searchedResult.driver_wins === 1 ? '' : 's'}`;
            }
        } catch(e) {
    ...
    async handle(handlerInput) {
        ...
        try {
            ...
            if (!searchedResult) {
                ...
                <say-as interpret-as="ordinal">${searchedResult.constructor_position}</say-as> in the championship, with ${searchedResult.constructor_points} point${searchedResult.constructor_points === 1 ? '' : 's'} and ${searchedResult.constructor_wins} win${searchedResult.constructor_wins === 1 ? '' : 's'}`;
            }
        } catch(e) {
        ...
        try {
            ...
            for (let i = 0; i < config.listMax; i++) {
                if (i < data.standings.length) {
                    speakOutput += `<amazon:emotion name="excited" intensity="medium"><break time="1s"/><say-as interpret-as="ordinal">${i + 1}</say-as> ${data.standings[i].driver_forename} ${data.standings[i].driver_surname} with ${data.standings[i].driver_points} point${data.standings[i].driver_points === 1 ? '' : 's'}. </amazon:emotion>`;
                }
            }
...
const FullChampionshipConstructorsIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        try {
            ...
            for (let i = 0; i < config.listMax; i++) {
                if (i < data.standings.length) {
                    speakOutput += `<amazon:emotion name="excited" intensity="medium"><break time="1s"/><say-as interpret-as="ordinal">${i + 1}</say-as> ${data.standings[i].constructor_name} with ${data.standings[i].constructor_points} point${data.standings[i].constructor_points === 1 ? '' : 's'}. </amazon:emotion>`;
                }
            }
            if (config.listMax < data.standings.length) {
                speakOutput += '<break time="1s"/>Would you like me to continue?';
        ...
        try {
            ...
            if (!year) {
                ...
                if (!driversResult.data.standings.length || !constructorsResult.data.standings.length) {
                    ...
                    ${driversResult.data.standings[0].driver_forename} ${driversResult.data.standings[0].driver_surname} with ${driversResult.data.standings[0].driver_points} point${driversResult.data.standings[0].driver_points === 1 ? '' : 's'}
                    and ${driversResult.data.standings[0].driver_wins} win${driversResult.data.standings[0].driver_wins === 1 ? '' : 's'}. The constructor's world champion was ${constructorsResult.data.standings[0].constructor_nationality} constructor
                    ${constructorsResult.data.standings[0].constructor_name} with ${constructorsResult.data.standings[0].constructor_points} point${constructorsResult.data.standings[0].constructor_points === 1 ? '' : 's'} and
                    ${constructorsResult.data.standings[0].constructor_wins} win${constructorsResult.data.standings[0].constructor_wins === 1 ? '' : 's'}.`;
                }
            }
            return handlerInput.responseBuilder
            .speak(speakOutput)
